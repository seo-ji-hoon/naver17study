...
public class Ex3OracleJoin {
	...
	public void joinWriteData() {
		
		Connection conn=null;
		Statement stmt=null;
		ResultSet rs=null;
		String sql="""
				select s.sangcode,sangname,sangprice,cntnum,to_char(cartday,'yyyy-mm-dd')cartday from shop s,cart c where s.sangcode=c.sangcode
				""";
		...
		try {
			...
			System.out.println("상품코드\t상품명\t가격\t수량\t등록일");
			...
			while(rs.next()) {
				...
				System.out.println(code+"\t"+sname+"\t"+price+"\t"+cnt+"\t"+cartday);
			}
		} catch (SQLException e) {
