	...
	public void initDesign() {
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//insert 메서드 호출
				shopModel.insertShop(new ShopDto(sampum,su,danga));
				//전체 데이타 다시 출력
				rowSelect();
				//입력한 데이타 초기화
				tfSang.setText("");
		...
		btnUpdate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				sampum=JOptionPane.showInputDialog("수정할 상품명을 입력하세요");
				danga=Integer.parseInt(JOptionPane.showInputDialog("수정할 단가를 입력하세요."));
				
				//수정 메서드 호출
				shopModel.updateShop(idx, sampum, danga);
				//테이블 다시 출력
				rowSelect();
			}
		});
		...
		btnSearch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(sampum.length()==0) {
					...
					//검색 결과 가져오기
					List<Vector<String>> searchList=shopModel.getSearchData(sampum);
					//테이블에 출력
