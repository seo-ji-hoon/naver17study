...
-- 모든 컬럼명 생략하고 순서대로 넣기 -> num은 시퀀스 컬럼이므로 null 로 줘도 숫자로 들어간다.
...
select round(avg(age),0) from person; -- 평균 나이 (정수타입으로)
select max(age) 최고나이, min(age) 최소나이 from person; -- 최고나이,최소나이 

-- 혈액형별 인원수와 평균나이를 구해보자
select blood,count(*) 인원수,round(avg(age),0) 평균나이 from person group by blood
order by blood asc;

-- limit 시작위치,가져올글의 갯수
select * from person limit 0,3; -- 첫글(0번)부터 3개만 가져오기 
select * from person limit 2,3; -- 2번글부터(시작은 0번부터) 3개만 가져오기 

-- where 조건과 limit 사용시
select * from person where age>=20 limit 1,2; -- 검색된 결과에서 순서 
