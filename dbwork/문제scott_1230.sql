...
select ename,hiredate,sal from emp where hiredate in(05);
...
select ename,job,sal from emp where ename LIKE '%A%' AND ename LIKE '%S%' AND ename LIKE '%M%';

--14. MGR 을 GROUP 으로 인원수와 평균 SAL 을 구하시오 (평균 SAL은 무조건 올림으로)
select count(*)인원수 ,ceil(avg(sal))평균연봉 from emp group by mgr;
